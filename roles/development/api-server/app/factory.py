...
def configure_error_handlers(app):
    ...
    def page_not_found(error):
        return failure_ret(msg="{} not found".format(request.url), code=404)
...
def configure_before_handlers(app):
    ...
    def before_all_requests():
        ...
        if config.LOG_REQUEST:
            logger.info(
                "request_id: {}, path: {}, method: {}, headers: {}, args: {}, data: {}, form: {}".format(
                    g.request_id,
                    request.path,
                    request.method,
                    # 处理参数中的中文打印乱码情况
    ...
    def after_request_callback(resp):
        if config.LOG_REQUEST:
            logger.info("request_id: {}, response: {}".format(g.request_id, json.dumps(resp.json or {}, ensure_ascii=False)))
