...
class _OpenStackDeploy(_Singleton):
    ...
    def __init__(self, run_command, run_cd):
        self.run_command = run_command  # 执行系统命令函数
        self.run_cd = run_cd  # 进入系统目录函数
    ...
    def run_stack_command(self, cmd):
        """进入stack用户执行命令
        """
        stack_cmd = 'su - %s -c "%s"' % (self._stack_user, cmd)
        return self.run_command(stack_cmd)

    def _check_os_type(self):
        return self.run_command("uname -s")

    def _check_os_version(self):
        return self.run_command("cat /etc/issue")

    def _check_os_memory(self):
        return self.run_command("free -g")

    def _check_os_disk(self):
        return self.run_command("df -Th")
    ...
    def update_soft(self):
        self.run_command("sudo apt -y autoremove")
    ...
    def create_user(self):
        ...
        self.run_command('echo "stack ALL=(ALL) NOPASSWD: ALL" | sudo tee /etc/sudoers.d/stack')
    ...
    def clone_devstack(self):
        ...
        self.run_stack_command(cmd)

    def update_local_conf(self):
    ...
    def uninstall(self):
        ...
        """
        with settings(warn_only=True):
            self.run_stack_command("cd %s && ./unstack.sh" % self._stack_root)
            self.run_stack_command("cd %s && ./clean.sh" % self._stack_root)
    ...
    def deploy(self):
        ...
        self.create_user()
        self.clone_devstack()
        self.disable_upgrade_pip()
        self.update_local_conf()
        self.install()
